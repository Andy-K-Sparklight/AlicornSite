...
> 什么是容器？ASC容器和MCX容器又是什么？有什么区别？

容器就是用来存放Minecraft核心还有其他的附属文件的一个具体的文件夹

MCX容器和其他启动器的存储方法类似，都是把核心和附属文件的本体放入文件夹中

ASC容器则是把所有文件放在另一文件夹，再通过链接将文件**投射**到文件夹中，好处是当你创建多个ASC容器的时候，大部分文件只会占用一次空间
